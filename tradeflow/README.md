...
- `sector_industry.csv` - Many-to-many join between `sector` and `industry` (`sector_id, industry_id, weight`) — see [PLAN.md](PLAN.md)
- `trade.csv` - Core trade flows, full Exiobase industry detail (trade_id, region1, region2, industry1, industry2, amount) — trade/trade_factor were never the file-size problem (see [PLAN.md](PLAN.md)'s revision note), so unlike interstate they have no separate Sector-level primary tier
...
**Import factors use Exiobase's M matrix** (total: direct + everything embodied in a sector's own upstream inputs, via the Leontief inverse), not the direct-only S matrix — matching EPA USEEIO's **import_emission_factors** methodology. Amounts stay in Euros (see Open work in PLAN.md for the USD conversion).
