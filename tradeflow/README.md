...
- `sector_industry.csv` - Many-to-many join between `sector` and `industry` (`sector_id, industry_id, weight`) — see [PLAN-industry.md](PLAN-industry.md)
- `trade.csv` - Core trade flows, full Exiobase industry detail (trade_id, region1, region2, industry1, industry2, amount) — trade/trade_factor were never the file-size problem (see [PLAN-industry.md](PLAN-industry.md)'s revision note), so unlike interstate they have no separate Sector-level primary tier
...
**Why `interstate` keeps `trade_id`:** `interstate_id`'s embedded `industry` is only a broad *category* used for state-allocation weighting, not the actual `sector1`→`sector2` pair. `trade_id` is the path back to the *shared originating international flow* (`trade.amount`, `trade.country`, `trade.flow_type`, exact `industry1`/`industry2`) — reliable specifically because the BEA/interstate pipeline only ever disaggregates one file (`country=US`, `flow_type=domestic`), so `trade_id` stays unique in that scope. `interstate_id` itself is `interstate`'s `PRIMARY KEY`, and the real `FOREIGN KEY` target for both `interstate_factor` and `interstate_estimate`.

**Dedup keys, not `trade_id`:** the true identity of a flow is its region/industry pair, not `trade_id`. `trade` has `UNIQUE(region1, region2, industry1, industry2)` (previously deduped on `trade_id, year, country, flow_type` — not safe, since the same physical flow can be pulled twice from two different country-perspective CSV runs, e.g. "imports to US" and "exports from Canada" both capture the same CA→US leg, under different `trade_id`/`country`/`flow_type`). `interstate` dedups on its own `interstate_id` (its `PRIMARY KEY`) — a separate `UNIQUE(state1, state2, sector1, sector2)` constraint was tried and removed: checked against real 2021 data, ~50,000 of 164,064 genuinely distinct `interstate_id` rows share a `(state1, state2, sector1, sector2)` tuple while differing only in `state_industry_code`, so that constraint silently rejected legitimate rows, not just true duplicates. `trade_id` isn't part of either dedup key, it's lineage only.
...
**`insert_interstate_rows` reads `interstate.csv` by header name, not fixed position** — a real bug fix: `interstate.csv`'s actual column order is `interstate_id, trade_id, state1, state2, sector1, sector2, state_industry_code, amount, commodity_code, industry_code, economic_multiplier`, which doesn't match `trade.csv`'s simpler layout. The old code read `region1`/`region2`/etc. from fixed positions borrowed from `trade.csv`, which silently misread every field once `interstate_id` shifted the real columns over by one. It now looks each column up by name, with position fallbacks only for files with no matching header.
...
**`industry`/`sector`/`sector_industry`: a many-to-many split, not a single FK column.** `interstate.csv` writes a small, primary (committed) version aggregated to BEA Sector level (`33DG`, `44RT`, `G`, ...) alongside a full-detail `-lg` sibling (Exiobase's native ~200-industry grain) — see [PLAN-industry.md](PLAN-industry.md) for the full design. (`trade.csv`/`trade_factor.csv` stay at full Exiobase industry detail always — they were never the file-size problem that motivated this split; only `interstate`/`interstate_factor`, from the state×state disaggregation, needed it.) `industry.csv` holds only the ~200 raw Exiobase rows (`industry_id, name, category`); `sector.csv` holds the 23 BEA Sector rows (`sector_id, name`); `sector_industry.csv` (`sector_id, industry_id, weight`) is the join between them. A single `industry.sector_id` column would have been simpler, but 16/200 Exiobase industries genuinely chain to more than one BEA Sector (checked empirically — up to 7 candidates for some, e.g. "Other business services"), so a true many-to-many join table is needed; `weight` (per `industry_id`, summing to 1.0) is how much of that industry's amount is attributed to each candidate sector, used to split `interstate` amounts proportionally across ambiguous sectors rather than picking one via majority vote. `interstate.sector1`/`sector2` has `fk_istate_sector1`/`fk_istate_sector2` foreign keys against `sector(sector_id)` (`trade.industry1`/`industry2` keeps its original `fk_trade_industry1`/`fk_trade_industry2` foreign keys against `industry(industry_id)`); `sector_industry` itself has FKs to both `sector(sector_id)` and `industry(industry_id)`.
