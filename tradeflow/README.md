...
**Dedup keys, not `trade_id`:** the true identity of a flow is its region/industry (or state/sector) pair, not `trade_id`. `trade` has `UNIQUE(region1, region2, industry1, industry2)` (previously deduped on `trade_id, year, country, flow_type` — not safe, since the same physical flow can be pulled twice from two different country-perspective CSV runs, e.g. "imports to US" and "exports from Canada" both capture the same CA→US leg, under different `trade_id`/`country`/`flow_type`). `interstate` has `UNIQUE(state1, state2, sector1, sector2)` (previously had no dedup at all) — `trade_id` isn't part of either dedup key, it's lineage only.
