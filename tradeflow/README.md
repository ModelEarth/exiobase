...
- `interstate_factor.csv` — state-to-state factor flows with `interstate_id`, `trade_id`, `factor_id`, `level`, and `flow_type`; joins to `trade.csv` through `trade_id` (see below — not through `interstate_id`, which `interstate.csv` doesn't have a column for) and to `factor.csv` through `factor_id`
...
**`interstate_id`** is a composite string built during state disaggregation (`bea/main_trade_analyzer.py`): `f"{year}-{trade_id}-US-{origin_state}-US-{dest_state}-{industry}"`. It plays the same structural role for `interstate`/`interstate_factor` that `trade_id` plays for `trade`/`trade_factor` (the row identity factor rows key off of) — but it has to be a string, not a plain integer, because one international `trade_id` fans out into many `interstate_id` rows (one per producing-state × consuming-state pair).

**Why `interstate_factor` keeps `trade_id`:** `interstate_id`'s embedded `industry` is a broad *category* used for state-allocation weighting, not the actual `industry1`→`industry2` pair, and `interstate.csv` has no `interstate_id` column to join through anyway. `trade_id` is the only reliable path back to `trade.industry1`/`trade.industry2` for a given interstate factor row — reliable specifically because the BEA/interstate pipeline only ever disaggregates one file (`country=US`, `flow_type=domestic`), so `trade_id` stays unique in that scope.
...
**`interstate.region1`/`region2` are named `state1`/`state2` in the database** — they're US state codes (CA, TX, ...), not Exiobase-style regions like `trade`'s. (The CSV pipeline's own internal DataFrame columns in `bea/main.py`/`bea/main_trade_analyzer.py` still use `region1`/`region2`; the Rust API reads `interstate.csv` by column *position*, not header name, so that's independent of the database column names and doesn't need to match.)

**One database per year:** the `year` column is being dropped from `trade`, `trade_factor`, and `interstate` in the database — a given Postgres database now holds exactly one year's data (`{EXIOBASE_NAME}_{year}`), so `year` is implicit in which database you're connected to rather than a per-row value. The CSV files themselves are unchanged and still carry a `year` column; it's just no longer written into these three tables when the API loads the CSVs.
